...
export GIRACOINCLI=${BUILDDIR}/qa/pull-tester/run-giracoin-cli
export GIRACOIND=${REAL_GIRACOIND}
...
if [ "x${ENABLE_GIRACOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and giracoind must all be enabled"
fi
