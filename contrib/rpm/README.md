...
The RPM spec file provided here is for Giracoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://giracoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Giracoin git repository](https://github.com/giracoin/giracoin) but are not part of
...
Currently only `Source10` is used, for the example `giracoin.conf` file.
...
in the giracoin git repository. At present only an SVG version of the Giracoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the giracoin
...
needed when 0.12.1 is released, a proper fix is already in the Giracoin git
master branch.
...
idea to build Giracoin Core against a static version of that library compiled
...
The `giracoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `giracoin-server` package. At this point, most RPM based
...
The `giracoin-server` package should have SELinux support. How to properly do
...
Giracoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/giracoin/). He has been
packaging Giracoin for Fedora at least since 2012.
...
4. I renamed the `giracoin` package that contains the Qt GUI to `giracoin-core` as
that appears to be how the general population refers to it, in contrast to
`giracoin-xt` or `giracoin-classic`. I wanted to make sure the general population
