...
###   This script attempts to download the signature file SHA256SUMS.asc from giracoin.org
...
WORKINGDIR="/tmp/giracoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://giracoin.org/bin/"
VERSIONPREFIX="giracoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'giracoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Giracoin Core binary release signing key installed?"
   fi
