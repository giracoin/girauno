...
bool CBlockTreeDB::LoadBlockIndexGuts(boost::function<CBlockIndex*(const uint256&)> insertBlockIndex)
{
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                if (pindexNew->IsProofOfWork())
                    if (pindexNew->nHeight >= 1000 && !CheckProofOfWork(pindexNew->GetBlockHash(), pindexNew->nBits, Params().GetConsensus()))
    ...
    /** [PoS implementation] chain trust (e.g. score) logic */
    ...
    {
        CBlockIndex* pindex = item.second;
        arith_uint256 nOldTrust = UintToArith256(pindex->pprev ? pindex->pprev->nChainTrust : uint256());
        pindex->nChainTrust = ArithToUint256(nOldTrust + UintToArith256(pindex->GetBlockTrust()));
    }
