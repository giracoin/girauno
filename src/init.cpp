 ...
 * between giracoind, and giracoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), GIRACOIN_CONF_FILENAME));
    if (mode == HMM_GIRACOIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), GIRACOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/giracoin/giracoin>";
    const std::string URL_WEBSITE = "<https://giracoincore.org>";
...
/** Sanity checks
 *  Ensure that Giracoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Giracoin process is using the data directory.
    ...
    {
#ifdef ENABLE_STAKING
        // standard BlockTreeDBCache math
    ...
    // ********************************************************* Step 12: start stake miner
#ifdef ENABLE_WALLET
#ifdef ENABLE_STAKING
    if (!fDisableWallet && pwalletMain && fTxIndex && GetBoolArg("-staking", fTxIndex && !fDisableWallet))
        threadGroup.create_thread(boost::bind(&ThreadStakeMiner, pwalletMain, Consensus::Params()));
#endif
#endif
    // ********************************************************* Step 13: finished
