...
#if defined(NDEBUG)
# error "Giracoin cannot be compiled without assertions."
#endif
...
/** [PoS implementation] stake-seen logic */
...
const string strMessageMagic = "Giracoin Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    {
        if (fDebug)
            LogPrintf("CheckTransaction(): coinbase check: scriptSig size=%d coinstake=%d\n", tx.vin[0].scriptSig.size(), tx.IsCoinStake());

        if (!tx.IsCoinStake() && (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 100))
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s GRC additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckBlockHeader(block, dummyState, consensusParams))
...
{
    if (nHeight == 1)
        return 2600 * 280797 * COIN; // block reward * block height before swap * units

    if (pindexBestHeader->nMoneySupply >= 21000000000 * COIN)
        return 0;
...
{
    if (chainActive.Height() == 0 && pindexBestHeader->nHeight == 0)
        return false;
    ...
    if (fCheckpointsEnabled && chainActive.Height() < Checkpoints::GetTotalBlocksEstimate(chainParams.Checkpoints()))
        return true;
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 90 * 3 || // more or less 3 hours of blocks
...
/** [PoS implementation] reward/diff logic */

bool GetCoinAge(const CTransaction tx, const CBlockIndex* pindexPrev, uint64_t& nCoinAge, const int64_t nPoSBlockTime, const Consensus::Params consensusParams)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        CDiskTxPos txpos;
        if (!pblocktree->ReadTxIndex(txin.prevout.hash, txpos))
            continue;  // previous transaction not in main chain

        // Read block header
        CBlock block;
        CDiskBlockPos blockpos(txpos.nFile, txpos.nPos);

        if (!ReadBlockFromDisk(block, blockpos, consensusParams))
            continue; // unable to read block header


        if (nPoSBlockTime < block.GetBlockTime())
            return false;  // Transaction timestamp violation

        int nSpendDepth;

        if (IsConfirmedInNPrevBlocks(txpos, pindexPrev, consensusParams.nStakeMinConfirmations - 1, nSpendDepth))
        {
        ...
        bnCentSecond += arith_uint256(nValueIn) * (nPoSBlockTime - block.GetBlockTime()) / CENT;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * 3 * CENT * 33 / (365 * 33 + 8);
    return nSubsidy + nFees;
}

int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, CBlockIndex* pindex)
{
    if (pindex->nMoneySupply >= 21000000000 * COIN)
        return 0;
...
/** Check whether coin stake tx inputs have enough confirmations */
bool IsConfirmedInNPrevBlocks(const CDiskTxPos& txpos, const CBlockIndex* pindexFrom, int nMaxDepth, int& nActualDepth)
{
    for (const CBlockIndex* pindex = pindexFrom; pindex && pindexFrom->nHeight - pindex->nHeight < nMaxDepth; pindex = pindex->pprev)
    {
        if (pindex->GetBlockPos().nPos == txpos.nPos && pindex->GetBlockPos().nFile == txpos.nFile)
        {
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0 && !tx.IsCoinStake()) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
				...
				if (fAddressIndex) {
                    // TODO check GetOutputFor for coinstake txs
...
void ThreadScriptCheck() {
    RenameThread("giracoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), !fJustCheck, !fJustCheck, block.IsProofOfStake()))
    ...
    /** BIP ENFORCEMENT
     * Here we skip most of the original Bitcoin conditions.
     * Being a new chain, most of the BIPs are enforced right
     * from the start
     * */

    // enforce BIP16 right from the start
    unsigned int flags = SCRIPT_VERIFY_P2SH;

    // enforce the DERSIG (BIP66) from the start
    flags |= SCRIPT_VERIFY_DERSIG;

    // enforce CHECKLOCKTIMEVERIFY, (BIP65) from the start
    ...
    /** [PoS implementation] - stake reward check logic - START */
    int64_t nValueIn = 0;
    int64_t nValueOut = 0;
    int64_t nStakeReward = 0;
    int64_t nCoinStakeValueIn = 0;
    /** [PoS implementation] - stake reward check logic - END */
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (tx.IsCoinStake() && fTxIndex)
            {
                ...
                for (unsigned int k = 0; k < tx.vin.size(); k++)
                {
                    ...
                    if (!pblocktree->ReadTxIndex(txin.prevout.hash, postx))
                        return state.DoS(100, error("ConnectBlock(): CoinStake input missing"), REJECT_INVALID, "bad-tx-coinstake-input-missing");
                    CBlock prevtxblock;
                    CDiskBlockPos blockpos(postx.nFile, postx.nPos);
                    if (!ReadBlockFromDisk(prevtxblock, blockpos, chainparams.GetConsensus()))
                        return state.DoS(100, error("ConnectBlock(): CoinStake input missing"), REJECT_INVALID, "bad-blk-coinstake-input-missing");
                    CTransaction txPrev;
                    unsigned int i = 0;
                    while (i < prevtxblock.vtx.size() && txPrev.IsNull())
                    {
                        if (prevtxblock.vtx[i].GetHash() == txin.prevout.hash)
                            txPrev = prevtxblock.vtx[i];
                        i++;
                    }
                    if (txPrev.IsNull())
                        return state.DoS(100, error("ConnectBlock(): CoinStake input missing"), REJECT_INVALID, "bad-tx-coinstake-input-null");

                    nTxValueIn += txPrev.vout[txin.prevout.n].nValue;
                    nCoinStakeValueIn += txPrev.vout[txin.prevout.n].nValue;
                }

                nValueIn += nTxValueIn; // used to track minting and supply
                nValueOut += nTxValueOut; // used to track minting and supply
            ...
            /** [Block Explorer implementation] - indexes - START */
            ...
            /** [Block Explorer implementation] - indexes - END */
        }
            ...
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash())
                {
                    ...
                    if (!tx.IsCoinStake())
                        ...
                        addressIndex.push_back(make_pair(CAddressIndexKey(2, uint160(hashBytes), pindex->nHeight, i, txhash, k, false), out.nValue - nCoinStakeValueIn));
    ...
    if (pindex->IsProofOfWork())
    {
        ...
        /** [PoS implementation] reward check - START */
    else if (pindex->IsProofOfStake() && fTxIndex) {
        uint64_t nCoinAge;
        if (!GetCoinAge(block.vtx[1], pindex->pprev, nCoinAge, block.GetBlockTime(), chainparams.GetConsensus()))
    ...
    /** [PoS implementation] reward check - END */
    ...
    /** [PoS implementation] record minted value - START */
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    /** [PoS implementation] record minted value - END */
    ...
    if (pindex->GetUndoPos().IsNull() || !pindex->IsValid(BLOCK_VALID_SCRIPTS))
    {
        ...
        // make sure there is consistency between CBlockIndex and CBlock structures
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block, bool fProofOfStake)
{
    ...
    /** [PoS implementation] compute chain trust score */
    arith_uint256 nOldTrust = UintToArith256(pindexNew->pprev ? pindexNew->pprev->nChainTrust : uint256());
    pindexNew->nChainTrust = ArithToUint256(nOldTrust + UintToArith256(pindexNew->GetBlockTrust()));

    if (pindexNew->IsProofOfStake())
        setStakeSeen.insert(make_pair(pindexNew->prevoutStake, pindexNew->GetBlockTime()));
...
{
    // we don't know if it is a PoW or PoS block, so we just check both
    if (!CheckProofOfWork(block.GetHash(), block.nBits, consensusParams))
        if (!CheckStakeSimple(block.GetHash(), block.nBits, consensusParams))
            return false;
...
bool SignBlock(CBlock* pblock, CWallet& wallet, int64_t nFees) {
    ...
    int64_t nSearchTime = pblock->GetBlockTime() & ~STAKE_TIMESTAMP_MASK;

    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        if (wallet.CreateCoinStake(wallet, pblock->nBits, nFees, txCoinStake, key, pblock->GetBlockTime(), Params().GetConsensus()))
        {
            if (pblock->GetBlockTime() >= pindexBestHeader->GetBlockTime() + 1)
            {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fProofOfStake)
{
    // These are checks that are independent of context.
    if (block.fChecked)
        return true;

    // This is mostly redundant with the call in AcceptBlockHeader.
    if (!CheckBlockHeader(block, state, consensusParams))
        return state.DoS(100, false, REJECT_INVALID, "bad-blk-header", false, "invalid header");
    ...
    /** [PoS implementation] sanity checks */
    if (fProofOfStake)
    {
        // Check timestamp
        if (block.GetBlockTime() > FutureDrift(GetAdjustedTime()))
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-blk-time", false, "block timestamp too far in the future");

        // Check proof-of-stake block signature
        if (!CheckBlockSignature(block))
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-blk-sign", false, "bad proof-of-stake block signature");

        // Coinbase output should be empty if proof-of-stake block
        if (block.vtx[0].vout.size() != 1 || !block.vtx[0].vout[0].IsEmpty())
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-blk-format", false, "coinbase output not empty for proof-of-stake block");

        // Second transaction must be coinstake, the rest must not be
        if (!block.vtx[1].IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-blk-format", false, "second tx is not coinstake");
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            if (block.vtx[i].IsCoinStake())
                return state.DoS(100, false, REJECT_INVALID, "bad-pos-blk-format", false, "more than one coinstake");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    // Check diff
    if (!CheckBlockHeader(block, state, consensusParams))
        return state.DoS(100, false, REJECT_INVALID, "bad-diffbits", false, "incorrect proof of work or proof of stake");
                              ...
                              const bool fProofOfStake = false)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // make sure there is consistency between CBlockIndex and CBlock structures
        if (fProofOfStake && pindex != NULL && !pindex->IsProofOfStake())
            pindex->SetProofOfStake();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake())
    {
        ...
        uint256 hashProof;
        if (!CheckProofOfStake(pindex->pprev, block.vtx[1], block.nBits, hashProof, block.GetBlockTime(), Params().GetConsensus()))
            return error("AcceptBlock() : check proof-of-stake failed for block %s", block.GetHash().ToString());
    }
    ...
    if ((!CheckBlock(block, state, chainparams.GetConsensus(), true, true, block.IsProofOfStake())) || !ContextualCheckBlock(block, state, pindex->pprev))
    {
...
{
    {
         ...
         * */
        if (!fReindex && !fImporting && pblock->IsProofOfStake() && setStakeSeen.count(pblock->GetProofOfStake()))
            return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for block %s",
                         pblock->GetProofOfStake().first.ToString(), pblock->GetProofOfStake().second, pblock->GetHash().ToString());
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW,
                       bool fCheckMerkleRoot, bool fProofOfStake)
{
    AssertLockHeld(cs_main);
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot, fProofOfStake))
        return error("%s: Consensus::CheckBlock: %s", __func__, FormatStateMessage(state));
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, chainparams.GetConsensus(), true, true, block.IsProofOfStake()))
...
bool RewindBlockIndex(const CChainParams& params)
{
    ...
    for (BlockMap::iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end(); it++) {
        ...
        if (IsWitnessEnabled(pindexIter->pprev, params.GetConsensus()) && !(pindexIter->nStatus & BLOCK_OPT_WITNESS) && !chainActive.Contains(pindexIter)) {
            ...
            pindexIter->nMint = 0;
            pindexIter->nFlags = 0;
            pindexIter->nMoneySupply = 0;
            pindexIter->prevoutStake.SetNull();
            pindexIter->nChainTrust = uint256();
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    if (fTxIndex)
        LogPrintf("Enabling txindex...\n");

    // Use the provided setting for -addressindex in the new database
	fAddressIndex = GetBoolArg("-addressindex", DEFAULT_ADDRESSINDEX);
	pblocktree->WriteFlag("addressindex", fAddressIndex);
    if (fAddressIndex)
	    LogPrintf("Enabling addressindex...\n");

	// Use the provided setting for -timestampindex in the new database
	fTimestampIndex = GetBoolArg("-timestampindex", DEFAULT_TIMESTAMPINDEX);
	pblocktree->WriteFlag("timestampindex", fTimestampIndex);
    if (fTimestampIndex)
	    LogPrintf("Enabling timestampindex...\n");

	fSpentIndex = GetBoolArg("-spentindex", DEFAULT_SPENTINDEX);
	pblocktree->WriteFlag("spentindex", fSpentIndex);
    if (fSpentIndex)
	    LogPrintf("Enabling spentindex...\n");
...
class CBlockSerializeHeaderHelper : public CBlockHeader
{
public:
    // network and disk
    std::vector<CTransaction> vtx;

    CBlockSerializeHeaderHelper()
    {
        SetNull();
    }

    CBlockSerializeHeaderHelper(const CBlockHeader &header)
    {
        SetNull();
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    else if (inv.type == MSG_CMPCT_BLOCK)
                    {
                        ...
                        if (mi->second->nHeight >= chainActive.Height() - 10 && !block.IsProofOfStake()) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        vector<CBlockSerializeHeaderHelper> vHeaders;
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/giracoin/giracoin/issues/8279 for details.
                            assert(recentRejects);
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        CBlockIndex *pindex = NULL;
        CValidationState state;
        if (!AcceptBlockHeader(cmpctblock.header, state, chainparams, &pindex, false)) {
            int nDoS;
            if (state.IsInvalid(nDoS)) {
                if (nDoS > 0)
                    Misbehaving(pfrom->GetId(), nDoS);
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        BOOST_FOREACH(const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                Misbehaving(pfrom->GetId(), 20);
                LogPrintf("NetMsgType::HEADERS\nhashPrevBlock=%s\npindexLast=%s\n", header.hashPrevBlock.ToString(), pindexLast->GetBlockHash().ToString());
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                if (vGetData.size() > 0) {
                    if (nodestate->fSupportsDesiredCmpctVersion && vGetData.size() == 1 && mapBlocksInFlight.size() == 1 && pindexLast->pprev->IsValid(BLOCK_VALID_CHAIN)) {
                        ...
                        /** [PoS implementation] we do need regular blocks most of the times */
