...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGiracoinAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-giracoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CGiracoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CGiracoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CGiracoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CGiracoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CGiracoinAddress(address).ToString());
}
...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        if ((unsigned int)pcoin.first->GetDepthInMainChain() >= Consensus::Params().nStakeMinConfirmations)
...
void CWallet::AvailableCoinsForStaking(vector<COutput>& vCoins) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            unsigned int nDepth = pcoin->GetDepthInMainChain();

            if (nDepth < Consensus::Params().nStakeMinConfirmations)
                continue;
            if (pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0)
                continue;
            ...
            {
                isminetype mine = IsMine(pcoin->vout[i]);
                if (!(IsSpent(wtxid, i)) && mine == ISMINE_SPENDABLE && pcoin->vout[i].nValue >= nMinimumInputValue)
                {
                    if (fDebug)
                        LogPrintf("AvailableCoinsForStaking(): vout found => nDepth=%d value=%d\n", nDepth, pcoin->vout[i].nValue);

                    vCoins.push_back(COutput(pcoin, i, nDepth, true, mine == ISMINE_SPENDABLE));
                }
            }
        }
    }
...
bool CWallet::CreateCoinStake(const CKeyStore &keystore, unsigned int nBits, int64_t nFees, CMutableTransaction &txNew, CKey &key, const int64_t nPoSBlockTime,
                              const Consensus::Params consensusParams)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n < 1 && !fKernelFound && pindexPrev == pindexBestHeader; n++)
        {
            ...
            if (CheckKernel(pindexPrev, nBits, nPoSBlockTime - n, prevoutStake, consensusParams, &nBlockTime))
            {
                // Found a kernel
    ...
    // Calculate coin age reward
    {
        ...
        if (!GetCoinAge(txNewConst, pindexPrev, nCoinAge, nPoSBlockTime, consensusParams))
    ...
    if (nBytes >= MAX_POS_BLOCK_SIZE/5)
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CGiracoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CGiracoinAddress(dest).ToString(), key);
}
